...
use std::arch::x86_64::{
    _mm256_permute2f128_si256 as permute2f128, _mm256_permute4x64_epi64 as permute4x64, _mm256_setr_epi32, _mm256_storeu_si256 as storeu, _mm256_unpackhi_epi64 as unpackhi64,
...
unsafe fn gentleman_sande_radix_4_kernel<M: Modulo>(deg: usize, width: usize, offset: usize, im: Modint<M>, a: &mut [Modint<M>], rate: &[Modint<M>]) {
    ...
    if offset == 1 && blocks >= 8 {
        ...
        for block in (0..blocks).step_by(2) {
            ...
            {
                let rot = Modintx8::load([rot, rot, rot, rot, next_rot, next_rot, next_rot, next_rot].as_ptr());
                let rot2 = rot * rot;
                let rot3 = rot * rot2;
                let (r0, r1, r2, r3) = radix4_inner!(
                    Modintx8::gather(a[top..].as_ptr(), vindex),
        ...
        for _ in (0..offset).step_by(8) {
            ...
            let (r0, r1, r2, r3) = radix4_inner!(Modintx8::load(c0a), Modintx8::load(c1a), Modintx8::load(c2a), Modintx8::load(c3a), imag);
            r0.store(c0a);
            r1.store(c1a);
            r2.store(c2a);
            r3.store(c3a);
        ...
        for block in 1..blocks {
            ...
            for _ in (top..top + offset).step_by(8) {
                let (c0a, c1a, c2a, c3a) = (head, head.add(offset), head.add(offset * 2), head.add(offset * 3));
                let (r0, r1, r2, r3) = radix4_inner!(Modintx8::load(c0a), Modintx8::load(c1a) * rot, Modintx8::load(c2a) * rot2, Modintx8::load(c3a) * rot3, imag);
                r0.store(c0a);
                r1.store(c1a);
                r2.store(c2a);
                r3.store(c3a);
            ...
            rot = rot * Modintx8::splat(*rate.get_unchecked((!block).trailing_zeros() as usize));
        }
    }
}
