...
unsafe fn gentleman_sande_radix_2_kernel<M: Modulo>(deg: usize, width: usize, offset: usize, a: &mut [Modint<M>], rate: &[Modint<M>]) {
    ...
    if offset == 1 && blocks >= 8 {
        ...
        for block in (0..blocks).step_by(8) {
            ...
            let c1 = Modintx8::gather_ptr(a[top + 1..].as_ptr(), vindex) * rot;
...
unsafe fn gentleman_sande_radix_4_kernel<M: Modulo>(deg: usize, width: usize, offset: usize, im: Modint<M>, a: &mut [Modint<M>], rate: &[Modint<M>]) {
    ...
    if offset == 1 && blocks >= 8 {
        ...
        for block in (0..blocks).step_by(8) {
            ...
            let rot = Modintx8::<M>::load(&r);
            let rot2 = rot * rot;
            let rot3 = rot * rot2;
            let (r0, r1, r2, r3) = radix4_inner!(
                Modintx8::gather_ptr(a[top..].as_ptr(), vindex),
                Modintx8::gather_ptr(a[top + 1..].as_ptr(), vindex) * rot,
                Modintx8::gather_ptr(a[top + 2..].as_ptr(), vindex) * rot2,
                Modintx8::gather_ptr(a[top + 3..].as_ptr(), vindex) * rot3,
                imag
            );
        ...
        for block in (0..blocks).step_by(4) {
            ...
            let (r0, r1, r2, r3) = radix4_inner!(
                Modintx8::gather_ptr(a[top..].as_ptr(), vindex),
                Modintx8::gather_ptr(a[top + 2..].as_ptr(), vindex) * rot,
                Modintx8::gather_ptr(a[top + 4..].as_ptr(), vindex) * rot2,
                Modintx8::gather_ptr(a[top + 6..].as_ptr(), vindex) * rot3,
                imag
            );
        ...
        for block in (0..blocks).step_by(2) {
            ...
            let next_rot = rot * rate[(!block).trailing_zeros() as usize];
            {
                ...
                let (r0, r1, r2, r3) = radix4_inner!(
                    Modintx8::gather_ptr(a[top..].as_ptr(), vindex),
                    Modintx8::gather_ptr(a[top + 4..].as_ptr(), vindex) * rot,
                    Modintx8::gather_ptr(a[top + 8..].as_ptr(), vindex) * rot2,
                    Modintx8::gather_ptr(a[top + 12..].as_ptr(), vindex) * rot3,
                    imag
                );
        ...
        for block in 0..blocks {
            ...
            let rot3 = rot * rot2;
            for now in top..top + offset {
                let (r0, r1, r2, r3) = radix4_inner!(a[now], a[now + offset] * rot, a[now + offset * 2] * rot2, a[now + offset * 3] * rot3, im);
                a[now] = r0;
                a[now + offset] = r1;
                a[now + offset * 2] = r2;
                a[now + offset * 3] = r3;
        ...
        for _ in (0..offset).step_by(8) {
            ...
            let (r0, r1, r2, r3) = radix4_inner!(Modintx8::load_ptr(c0a), Modintx8::load_ptr(c1a), Modintx8::load_ptr(c2a), Modintx8::load_ptr(c3a), imag);
            r0.store_ptr(c0a);
            r1.store_ptr(c1a);
            r2.store_ptr(c2a);
            r3.store_ptr(c3a);
        ...
        let mut rot = Modintx8::splat_raw(rate[0]);
        for block in 1..blocks {
            ...
            for _ in (top..top + offset).step_by(8) {
                ...
                let (r0, r1, r2, r3) = radix4_inner!(
                    ...
                    Modintx8::load_ptr(c1a) * rot,
                    Modintx8::load_ptr(c2a) * rot2,
                    Modintx8::load_ptr(c3a) * rot3,
                    imag
                );
                r0.store_ptr(c0a);
                r1.store_ptr(c1a);
            ...
            rot = rot * Modintx8::splat_raw(*rate.get_unchecked((!block).trailing_zeros() as usize));
        }
    }
}
...
#[inline]
pub unsafe fn gentleman_sande_radix_4_butterfly_inv<M: Modulo>(deg: usize, log: usize, a: &mut [MontgomeryModint<M>], cache: &FftCache<M>) {
    for i in (0..log).step_by(2) {
        let width = deg >> i;
        if i + 1 == log {
            let offset = width >> 1;
